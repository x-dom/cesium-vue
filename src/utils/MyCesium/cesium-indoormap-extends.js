...
Object.defineProperties(IndoorBuilding.prototype, {
    show: {
        ...
        set: function(bool) {
            let _this = this;
            _this._show = bool;
            _this.floors.forEach((el,index) => {el.floor.show = _this._show});
        }
    },
});
...
const IndoorFloor = function (viewer, options) {
    let defaultOptions = {
        ...
        roomColor_express: function() {
            return Cesium.Color.fromCssColorString(this.selectColor);
            if(this.select){
                return Cesium.Color.fromRandom().withAlpha(1.0);
            } else {
                return Cesium.Color.fromRandom().withAlpha(0.5);
            }
        }
    };
    this.viewer = viewer;
